...
local_stashdb_tags = df_stash_tags.filter(
    pl.col("stashdb_id").is_in(set(local_stashdb_ids) - set(remote_stashdb_ids))
)
local_stashdb_tags
...
df_description_updates = (
    pl.DataFrame(description_update_records)
    .sort(["name", "field"])
    .filter(pl.col("current_value") != pl.col("proposed_value"))
)
df_description_updates
...
for row in df_description_updates.iter_rows(named=True):
    ...
    update_data = {"id": row["tag_id"], "description": row["proposed_value"]}
    try:
...
for row in df_stash_tags.iter_rows(named=True):
    ...
    if not stashdb_tag.is_empty():
        ...
        proposed_aliases = {
            alias for alias in (stashdb_tag["aliases"] or []) if not contains_cjk(alias)
        }
        ...
        if current_regular_aliases != proposed_aliases:
            ...
            final_stashdb_id = (
                next(iter(current_stashdb_ids)) if current_stashdb_ids else None
            )
...
for row in df_alias_updates.iter_rows(named=True):
    update_data = {"id": row["tag_id"], "aliases": row["proposed_list"]}
    try:
        stash.update_tag(update_data)
        print(f"Updated tag: {row['name']}")
    except Exception as e:
...
df_stash_tags = df_stash_tags.with_columns(
    pl.col("aliases")
    .map_elements(
        lambda x: [alias for alias in x if not contains_cjk(alias)],
        return_dtype=pl.List(pl.Utf8),
    )
    .alias("cleaned_aliases")
)
...
print(tags_to_update.select(["name", "aliases", "cleaned_aliases"]).head())


# Optional: Apply the updates
def apply_alias_cleanup(tags_df):
    for row in tags_df.iter_rows(named=True):
        update_data = {"id": row["id"], "aliases": row["cleaned_aliases"]}
...
tags_to_update_for_review = tags_to_update.select(
...
# %%
stashdb_only_tags = df_stashdb_tags.filter(
    ~pl.col("id").is_in(df_stash_tags.select("stashdb_id").to_series())
)
stashdb_only_tags
...
for tag in already_existing_tags:
    ...
    stash.update_tag({"id": stash_tag["id"], "aliases": updated_aliases})
