...
class PlaywrightExample
{
    ...
    public static int Main(string[] args)
    {
        ...
        return Parser.Default.ParseArguments<ScrapeOptions, DownloadOptions, UpsizeOptions>(args)
          .MapResult(
            ...
            (UpsizeOptions opts) => RunUpsizeAndReturnExitCode(opts).GetAwaiter().GetResult(),
            errs => 1);
    }

    private static async Task<int> RunScrapeAndReturnExitCode(ScrapeOptions opts)
    {
        try
        {
    ...
    private static async Task<int> RunDownloadAndReturnExitCode(DownloadOptions opts)
    {
        try
        {
            InitializeLogger(opts);
    ...
    private static async Task<int> RunUpsizeAndReturnExitCode(UpsizeOptions opts)
    {
        try
        {
            InitializeLogger(opts);
