...
scene_renames_df = scene_renames_df.filter(pl.col("old_filename") != pl.col("new_filename"))

# Add check for duplicate new filenames
duplicate_filenames = scene_renames_df.group_by("new_filename").agg([
    pl.len().alias("count"),
...
print(f"Rename succeeded for {len(scenes_success_df)}/{len(scene_renames_df)} scenes, failed for {len(scenes_failed_df)}.")
