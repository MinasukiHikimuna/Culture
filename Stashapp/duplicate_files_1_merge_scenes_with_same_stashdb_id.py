...
all_scenes_basic_info = stash.find_scenes({}, fragment="id title studio { id name } stash_ids { endpoint stash_id }")
...
if len(scenes_with_dupe_stashdb_ids_basic_info) > 0:
    ...
    all_scenes = stash.find_scenes_by_studio(studio_id)
...
for group in grouped_scenes.iter_rows(named=True):
    ...
    scenes = [stash_client.find_scene(str(scene_id)) for scene_id in scene_ids]
    if not scenes:
    ...
    try:
        ...
        result = stash_client.call_GQL(query, {"merge_input": scene_merge_input})
